...
describe("HikeRecords test suite", () => {
    ...
    describe("Start an hike", () => {
        test("successful start of an hike", async () => {
            ...
            expect(records[0].userID).toBe(toBeInsertedRecord.userID)
            expect(records[0].hikeID).toBe(toBeInsertedRecord.hikeID)
            expect(records[0].status).toBe("open")
            expect(dayjs(records[0].startDate).format()).toBe(toBeInsertedRecord.startDate)
    ...
    /* 
        ...
        describe("Get all hikes", () => {
            test("", async () => {
                await hikeRecordsController.getRecords()
            })
            test("", async () => {
        ...
        describe("Get all open hikes", () => {
            test("", async () => {
                await hikeRecordsController.getRecordByStatusOpen()
            })
            test("", async () => {
